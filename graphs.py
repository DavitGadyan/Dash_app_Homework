...
trace1=go.Bar(x=[15,50,15,20],y=["x8","x7","x6","x5"],orientation = 'h',marker=dict(color="red"),name="<b>Negative</b>")
trace2=go.Bar(x=[-15,-45,-5,-35],y=["x4","x3","x2","x1"],orientation = 'h',marker=dict(color="blue"),name="Positive")
layout  = dict(title="<b>Correlation with employees probability of churn</b>",yaxis=dict(title="Variable"),showlegend=True)
...
#Graph 2
import quandl
quandl.ApiConfig.api_key = "sSdDMgdDB-MzBCFixSSD"
...
trace_1 = go.Box(x=mydata1.Open.pct_change(),name="<b>Google</b>")
trace_2 = go.Box(x=mydata2.Open.pct_change(),name="<b>Bitcoin</b>")
layout=dict(title="Distribution of Price Change")
data = [trace_2,trace_1]
f3 = dict(data=data,layout=layout)
